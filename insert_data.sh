 ...
 do
  if [[ $WINNER != 'winner' ]]
    then
      ...
      #insert into games
      INSERT_GAMES_RES=$($PSQL "INSERT INTO games(year,round,winner_id,opponent_id,winner_goals,opponent_goals) VALUES($YEAR, '$ROUND', $WINNER_ID, $OPPONENT_ID, $WINNER_GOALS, $OPPONENT_GOALS)")
       if [[ $INSERT_GAMES_RES == "INSERT 0 1" ]]
    then
      echo Inserted into games, $ROUND $WINNER x $OPPONENT: $WINNER_GOALS  $OPPONENT_GOALS
    fi
  fi  
 done
